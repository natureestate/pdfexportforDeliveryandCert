        ...
        <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-indigo-50">
            ...
            <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
                ...
                <div className="text-center mb-12">
                    ...
                    <div className="mt-8 inline-flex items-center gap-3 p-1.5 bg-gray-100 rounded-xl">
                        <button
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full border-collapse">
                            ...
                            <tbody className="divide-y divide-gray-100">
                                ...
                                {/* Users */}
                                <tr>
                                    <td className="py-3 px-4 text-gray-600">จำนวนผู้ใช้งาน</td>
                                    {plans.map((plan) => (
                                ...
                                {/* Documents */}
                                <tr className="bg-gray-50">
                                    <td className="py-3 px-4 text-gray-600">เอกสารที่สร้างได้/เดือน</td>
                                    {plans.map((plan) => (
                                        <td key={plan.id} className="py-3 px-4 text-center">
                                ...
                                {/* Document Types */}
                                <tr>
                                    <td className="py-3 px-4 text-gray-600">ประเภทเอกสารที่ใช้ได้</td>
                                    {plans.map((plan) => (
                                ...
                                {/* Custom Logo */}
                                <tr className="bg-gray-50">
                                    <td className="py-3 px-4 text-gray-600">โลโก้บนเอกสาร</td>
                                    {plans.map((plan) => (
                                        <td key={plan.id} className="py-3 px-4 text-center">
                                ...
                                {/* Watermark */}
                                <tr>
                                    <td className="py-3 px-4 text-gray-600">ลายน้ำ (Watermark)</td>
                                    {plans.map((plan) => (
                                        <td key={plan.id} className="py-3 px-4 text-center">
                                            {plan.features.hasWatermark ? (
                                                <span className="text-amber-600">มีลายน้ำ App</span>
                                            ) : (
                                                <span className="text-green-600">❌ ไม่มีลายน้ำ</span>
                                            )}
                                        </td>
                                    ))}
                                </tr>
                                ...
                                {/* CRM */}
                                <tr className="bg-gray-50">
                                    <td className="py-3 px-4 text-gray-600">ฐานข้อมูลลูกค้า (CRM)</td>
                                    {plans.map((plan) => (
                                        <td key={plan.id} className="py-3 px-4 text-center">
                                ...
                                {/* Contractors */}
                                <tr>
                                    <td className="py-3 px-4 text-gray-600">ฐานข้อมูลช่าง/ผู้รับเหมา</td>
                                    {plans.map((plan) => (
                                        <td key={plan.id} className="py-3 px-4 text-center">
                                ...
                                {/* PDF Export */}
                                <tr className="bg-gray-50">
                                    <td className="py-3 px-4 text-gray-600">ส่งออก PDF</td>
                                    {plans.map((plan) => (
                                        <td key={plan.id} className="py-3 px-4 text-center">
                                ...
                                {/* History */}
                                <tr>
                                    <td className="py-3 px-4 text-gray-600">ประวัติเอกสาร (Log)</td>
                                    {plans.map((plan) => (
                                        <td key={plan.id} className="py-3 px-4 text-center">
                                ...
                                {/* Reports */}
                                <tr className="bg-gray-50">
                                    <td className="py-3 px-4 text-gray-600">รายงาน (Dashboard)</td>
                                    {plans.map((plan) => (
                                        <td key={plan.id} className="py-3 px-4 text-center text-sm">
                                ...
                                {/* Support */}
                                <tr>
                                    <td className="py-3 px-4 text-gray-600">การสนับสนุน (Support)</td>
                                    {plans.map((plan) => (
                                        <td key={plan.id} className="py-3 px-4 text-center text-sm">
                ...
                <div className="mt-16 text-center">
                    <div className="inline-flex flex-col sm:flex-row items-center gap-4 p-6 bg-gradient-to-r from-indigo-50 to-blue-50 rounded-2xl">
                        ...
                        <a
                            ...
                            className="flex items-center gap-2 px-5 py-2.5 bg-white rounded-xl text-gray-700 hover:text-indigo-600 transition-colors shadow-sm"
                        >
                            ติดต่อเรา
