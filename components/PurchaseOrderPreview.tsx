        ...
        <div ref={ref} className="bg-white shadow-lg p-8 md:p-12 w-full aspect-[210/297] overflow-auto text-sm" id="printable-area">
            <header className="flex justify-between items-start pb-4 border-b border-gray-400">
                ...
                <div className="w-3/5 text-right">
                    ...
                    <h2 className="text-lg text-gray-500">PURCHASE ORDER</h2>
                    <div className="mt-4 text-xs space-y-1 text-gray-700">
                        <p><span className="font-semibold text-gray-600">เลขที่:</span> <span className="text-gray-800">{data.purchaseOrderNumber || '________________'}</span></p>
                        <p><span className="font-semibold text-gray-600">วันที่ออก:</span> <span className="text-gray-800">{formatDate(data.purchaseOrderDate)}</span></p>
                        {data.expectedDeliveryDate && (
                            <p><span className="font-semibold text-gray-600">วันที่ต้องการรับสินค้า:</span> <span className="text-gray-800">{formatDate(data.expectedDeliveryDate)}</span></p>
                        )}
