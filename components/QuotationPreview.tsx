        ...
        <div ref={ref} className="bg-white shadow-lg p-8 md:p-12 w-full aspect-[210/297] overflow-auto text-sm" id="printable-area">
            <header className="flex justify-between items-start pb-4 border-b border-gray-400">
                ...
                <div className="w-3/5 text-right">
                    <h1 className="text-2xl font-bold text-gray-800">ใบเสนอราคา</h1>
                    <h2 className="text-lg text-gray-500">QUOTATION</h2>
                    <div className="mt-4 text-xs space-y-1 text-gray-700">
                        <p><span className="font-semibold text-gray-600">เลขที่:</span> <span className="text-gray-800">{data.quotationNumber || '________________'}</span></p>
                        <p><span className="font-semibold text-gray-600">วันที่ออก:</span> <span className="text-gray-800">{formatDate(data.quotationDate)}</span></p>
                        {data.validUntilDate && (
                            <p><span className="font-semibold text-gray-600">วันที่หมดอายุ:</span> <span className="text-gray-800">{formatDate(data.validUntilDate)}</span></p>
                        )}
            ...
            {/* เงื่อนไขการชำระเงิน การส่งมอบ และหมายเหตุ */}
            {(data.paymentTerms || data.deliveryTerms || data.notes) && (
                <section className="mb-6 space-y-2">
                    {data.paymentTerms && (
                        <div>
                            <p className="font-semibold text-slate-600 text-sm">เงื่อนไขการชำระเงิน:</p>
                            <p className="text-slate-700 whitespace-pre-wrap text-xs">{data.paymentTerms}</p>
                        </div>
                    )}
                    {data.deliveryTerms && (
                        <div>
                            <p className="font-semibold text-slate-600 text-sm">เงื่อนไขการส่งมอบ:</p>
                            <p className="text-slate-700 whitespace-pre-wrap text-xs">{data.deliveryTerms}</p>
                        </div>
                    )}
                    {data.notes && (
                        <div>
