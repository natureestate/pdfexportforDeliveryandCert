        ...
        <div className="space-y-8 pt-4">
            ...
            <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4">
                    ...
                    <div>
                        <label htmlFor="projectName" className="block text-xs sm:text-sm font-medium text-slate-700">โครงการ / ลูกค้า</label>
                        <input type="text" id="projectName" value={data.projectName} onChange={(e) => handleDataChange('projectName', e.target.value)} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-xs sm:text-sm bg-gray-50" />
                    </div>
                    <div>
                        <label htmlFor="location" className="block text-xs sm:text-sm font-medium text-slate-700">สถานที่</label>
                        <input type="text" id="location" value={data.location} onChange={(e) => handleDataChange('location', e.target.value)} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-xs sm:text-sm bg-gray-50" />
                    </div>
                    <div>
                        <label htmlFor="contractNumber" className="block text-xs sm:text-sm font-medium text-slate-700">อ้างอิงสัญญาเลขที่</label>
                        <input type="text" id="contractNumber" value={data.contractNumber || ''} onChange={(e) => handleDataChange('contractNumber', e.target.value)} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-xs sm:text-sm bg-gray-50" />
                    </div>
                    <div>
                        <label htmlFor="requestedBy" className="block text-xs sm:text-sm font-medium text-slate-700">ผู้ร้องขอ</label>
                        <select id="requestedBy" value={data.requestedBy} onChange={(e) => handleDataChange('requestedBy', e.target.value as 'customer' | 'company' | 'designer')} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-xs sm:text-sm bg-gray-50">
                ...
                <div className="space-y-4">
                    <div>
                        ...
                        <input type="text" id="subject" value={data.subject} onChange={(e) => handleDataChange('subject', e.target.value)} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-xs sm:text-sm bg-gray-50" placeholder="เช่น ขอเปลี่ยนแปลงสเปคกระเบื้องห้องน้ำชั้น 2" />
                    </div>
                    <div>
                        <label htmlFor="originalScope" className="block text-xs sm:text-sm font-medium text-slate-700">รายละเอียดงานเดิม (Original Scope / Spec)</label>
                        <textarea id="originalScope" value={data.originalScope} onChange={(e) => handleDataChange('originalScope', e.target.value)} rows={3} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-xs sm:text-sm bg-gray-50" />
                    </div>
                    <div>
                        <label htmlFor="newScope" className="block text-xs sm:text-sm font-medium text-slate-700">รายละเอียดงานใหม่ (New Scope / Spec)</label>
                        <textarea id="newScope" value={data.newScope} onChange={(e) => handleDataChange('newScope', e.target.value)} rows={3} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-xs sm:text-sm bg-gray-50" />
                    </div>
                    <div>
                        <label htmlFor="reasonForChange" className="block text-xs sm:text-sm font-medium text-slate-700">เหตุผลในการเปลี่ยนแปลง</label>
                        <textarea id="reasonForChange" value={data.reasonForChange} onChange={(e) => handleDataChange('reasonForChange', e.target.value)} rows={2} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-xs sm:text-sm bg-gray-50" />
                    </div>
                </div>
                ...
                <div className="overflow-x-auto -mx-3 sm:mx-0">
                    <table className="min-w-full divide-y divide-gray-200 text-xs sm:text-sm">
                        <thead className="bg-gray-50">
                            <tr>
                                <th scope="col" className="px-2 sm:px-3 py-1.5 sm:py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-1/12">ประเภท</th>
                        ...
                        <tbody className="bg-white divide-y divide-gray-200">
                            {data.items.map((item, index) => (
                                <tr key={index} className={item.itemType === 'new' ? 'bg-green-50' : 'bg-red-50'}>
                                    <td className="px-1 sm:px-2 py-1 whitespace-nowrap">
                                        <span className={`px-2 py-1 text-xs font-medium rounded ${item.itemType === 'new' ? 'bg-green-200 text-green-800' : 'bg-red-200 text-red-800'}`}>
                                            {item.itemType === 'new' ? 'งานใหม่' : 'งานเดิม'}
                                        </span>
                                    </td>
                                    <td className="px-1 sm:px-2 py-1 whitespace-nowrap">
                                        <textarea value={item.description} onChange={(e) => handleItemChange(index, 'description', e.target.value)} rows={2} className="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring-indigo-200 focus:ring-opacity-50 text-xs sm:text-sm bg-white text-gray-900"></textarea>
                                    </td>
                                    <td className="px-1 sm:px-2 py-1 whitespace-nowrap">
                                        <input type="number" value={item.quantity} onChange={(e) => handleItemChange(index, 'quantity', parseFloat(e.target.value) || 0)} className="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring-indigo-200 focus:ring-opacity-50 text-xs sm:text-sm bg-white text-gray-900" />
                                    </td>
                                    <td className="px-1 sm:px-2 py-1 whitespace-nowrap">
                                        <input type="text" value={item.unit} onChange={(e) => handleItemChange(index, 'unit', e.target.value)} className="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring-indigo-200 focus:ring-opacity-50 text-xs sm:text-sm bg-white text-gray-900" />
                                    </td>
                                    <td className="px-1 sm:px-2 py-1 whitespace-nowrap">
                                        <input type="number" step="0.01" value={item.unitPrice} onChange={(e) => handleItemChange(index, 'unitPrice', parseFloat(e.target.value) || 0)} className="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring-indigo-200 focus:ring-opacity-50 text-xs sm:text-sm bg-white text-gray-900" />
                                    </td>
                                    <td className="px-1 sm:px-2 py-1 whitespace-nowrap">
                                        <input type="number" step="0.01" value={item.amount} readOnly className={`w-full rounded-md border-gray-300 shadow-sm text-xs sm:text-sm font-medium ${item.itemType === 'new' ? 'bg-green-100 text-green-900' : 'bg-red-100 text-red-900'}`} />
                                    </td>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4">
                    <div className="space-y-3 sm:space-y-4">
                        ...
                        <div>
                            <label htmlFor="paymentNote" className="block text-xs sm:text-sm font-medium text-slate-700">หมายเหตุการชำระเงิน</label>
                            <input type="text" id="paymentNote" value={data.paymentNote || ''} onChange={(e) => handleDataChange('paymentNote', e.target.value)} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-xs sm:text-sm bg-gray-50" placeholder="เช่น หักจากงวดที่ 10" />
                        </div>
                    </div>
                    <div className="space-y-2 bg-gray-50 p-4 rounded-lg">
                        <div className="flex justify-between">
                            <span className="text-sm text-gray-600">ยอดรวมงานใหม่/งานเพิ่ม:</span>
                            <span className="text-sm font-medium text-green-600">{data.newItemsSubtotal.toLocaleString('th-TH', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} บาท</span>
                        </div>
                        <div className="flex justify-between">
                            <span className="text-sm text-gray-600">ยอดรวมงานเดิม/งานลด:</span>
                            <span className="text-sm font-medium text-red-600">-{data.deductItemsSubtotal.toLocaleString('th-TH', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} บาท</span>
                        </div>
                        <div className="flex justify-between pt-2 border-t border-gray-300">
                            <span className="text-sm font-semibold text-gray-900">ยอดรวมส่วนต่าง (สุทธิ):</span>
                            <span className={`text-sm font-bold ${data.netDifference >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                        ...
                        <div className="flex justify-between pt-2 border-t border-gray-300">
                            <span className="text-base font-semibold text-gray-900">ยอดรวมที่ต้องชำระเพิ่ม/หัก:</span>
                            <span className={`text-base font-bold ${data.totalAmount >= 0 ? 'text-indigo-600' : 'text-red-600'}`}>
                ...
                <div className="space-y-4">
                    <div className="flex items-center">
                        ...
                        <label htmlFor="hasTimeImpact" className="ml-2 block text-xs sm:text-sm font-medium text-slate-700">มีผลกระทบต่อระยะเวลาโครงการ</label>
                    </div>
                    {data.hasTimeImpact && (
                        <>
                            <div>
                                <label htmlFor="timeImpactDays" className="block text-xs sm:text-sm font-medium text-slate-700">จำนวนวันที่ขยายออกไป (วันทำการ)</label>
                                <input type="number" id="timeImpactDays" value={data.timeImpactDays || 0} onChange={(e) => handleDataChange('timeImpactDays', parseFloat(e.target.value) || 0)} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-xs sm:text-sm bg-gray-50" />
                            </div>
                            <div>
                                <label htmlFor="timeImpactReason" className="block text-xs sm:text-sm font-medium text-slate-700">เหตุผล</label>
                                <textarea id="timeImpactReason" value={data.timeImpactReason || ''} onChange={(e) => handleDataChange('timeImpactReason', e.target.value)} rows={2} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-xs sm:text-sm bg-gray-50" />
                            </div>
                        </>
                    )}
                </div>
                ...
                <div className="space-y-4">
                    <div>
                        ...
                        <textarea id="terms" value={data.terms || ''} onChange={(e) => handleDataChange('terms', e.target.value)} rows={2} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-xs sm:text-sm bg-gray-50" placeholder="เช่น ราคานี้มีผลยืนยันภายใน 7 วัน" />
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4">
                        <div>
                            <label htmlFor="customerApproverName" className="block text-xs sm:text-sm font-medium text-slate-700">ชื่อผู้อนุมัติ (ลูกค้า)</label>
                            <input type="text" id="customerApproverName" value={data.customerApproverName || ''} onChange={(e) => handleDataChange('customerApproverName', e.target.value)} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-xs sm:text-sm bg-gray-50" />
                        </div>
                        <div>
                            <label htmlFor="customerApproverDate" className="block text-xs sm:text-sm font-medium text-slate-700">วันที่อนุมัติ (ลูกค้า)</label>
                            <input type="date" id="customerApproverDate" value={formatDateForInput(data.customerApproverDate)} onChange={(e) => handleDataChange('customerApproverDate', e.target.value ? new Date(e.target.value) : null)} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-xs sm:text-sm bg-gray-50" />
                        </div>
                        <div>
                            <label htmlFor="companyApproverName" className="block text-xs sm:text-sm font-medium text-slate-700">ชื่อผู้เสนอ (บริษัท)</label>
                            <input type="text" id="companyApproverName" value={data.companyApproverName || ''} onChange={(e) => handleDataChange('companyApproverName', e.target.value)} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-xs sm:text-sm bg-gray-50" />
                        </div>
                        <div>
                            <label htmlFor="companyApproverDate" className="block text-xs sm:text-sm font-medium text-slate-700">วันที่เสนอ (บริษัท)</label>
                            <input type="date" id="companyApproverDate" value={formatDateForInput(data.companyApproverDate)} onChange={(e) => handleDataChange('companyApproverDate', e.target.value ? new Date(e.target.value) : null)} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-xs sm:text-sm bg-gray-50" />
                        </div>
                    </div>
                </div>
