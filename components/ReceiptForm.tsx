...
const ReceiptForm: React.FC<ReceiptFormProps> = ({ 
    ...
    // ใช้ useDocumentForm hook แทน logic ที่ซ้ำกัน (พร้อม changeAmount สำหรับใบเสร็จ)
    const { handleDataChange, handleItemChange, addItem, removeItem: removeItemFromForm, calculateTotals } = useDocumentForm<ReceiptData, ReceiptItem>({
        data,
        setData,
        createNewItem: () => ({
            description: '',
        ...
        computeExtraTotals: (_subtotal, _taxAmount, total, currentData) => ({
            changeAmount: Math.max(0, currentData.paidAmount - total),
     ...
     */
    useEffect(() => {
        const SESSION_KEY = 'receipt_docNumber';
        ...
        if (hasValidNumber) {
            ...
            hasGeneratedNumberRef.current = true;
            sessionStorage.setItem(SESSION_KEY, data.receiptNumber);
            return;
        }
        
        // ตรวจสอบ sessionStorage ว่ามีเลขที่ generate ไว้แล้วหรือไม่
        const savedDocNumber = sessionStorage.getItem(SESSION_KEY);
        if (savedDocNumber && savedDocNumber.match(/^RC-\d{6}\d{2}$/)) {
            handleDataChange('receiptNumber', savedDocNumber);
            hasGeneratedNumberRef.current = true;
            return;
        }
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4">
                    <div>
                        ...
                        <DatePicker
                            ...
                            onChange={(date) => handleDataChange('receiptDate', date)}
